...
5. Route by `last_event` to pick up where the session left off:
   ...
   | `reviewers-clean` | `CODE-HUMAN-GATE` | Wait for human signal. **Approved (merge confirmed):** fire `done`. **Changes requested:** surface `review record --direct-clean-file/--report` audit risk first — without a matching `--operation-id` both are non-idempotent, and a replay may double-increment `review_round_count` and overwrite one level of fingerprint audit history; explicit human authorization required before any such replay; if authorized, read `last_review_clean_source` from `state.json` and replay that form — `"direct-clean"` → `--direct-clean-file <raw-path>`, `"report"` → `--report <adjudication-path> --adjudication` — passing `--operation-id <run_id>:<transition_sequence>`. That makes the replay a no-op only when `last_review_record_operation_id` already equals that id and the artifact the form names is still on disk; a clean round recorded flaglessly, or one whose gitignored artifact has been evicted, still refuses or still counts a round, so the authorization above is not optional; `null` means no clean round was recorded, so Surface rather than guess; then fire `blocker-applied` → apply fix → fire `wave-complete` → re-run GATES → REVIEW (adversarial first) |
